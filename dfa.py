...
def extract_dfa_transitions_with_trash_expanded(formula, regions=None):
    # pass `regions` so the condition strings come out in the same atom order
    # as the belief grid. spot orders atoms by where they appear in the formula,
    # which usually does not match. without this the string-equality check later
    # silently fails and every transition probability ends up 0.
    # cost me an hours, leaving the note here.
    ...
    spot_aps = [str(ap) for ap in dfa.ap()]
    if regions is None:
        atomic_props = spot_aps
    else:
        atomic_props = list(regions)
    spot_ap_set = set(spot_aps)
